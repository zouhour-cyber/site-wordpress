...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the Travelstore_Breadcrumb_Trail class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to Travelstore_Breadcrumb_Trail.
 * @return string
 */
function travelstore_breadcrumb_trail( $args = array() ) {
	$breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );
	if ( ! is_object( $breadcrumb ) ) {
		$breadcrumb = new Travelstore_Breadcrumb_Trail( $args );
	}
 ...
 * @access public
 */
class Travelstore_Breadcrumb_Trail {
	 ...
	 * Sets up the breadcrumb trail properties.  Calls the `Travelstore_Breadcrumb_Trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see Travelstore_Breadcrumb_Trail::set_labels()

	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Travelstore_Breadcrumb_Trail::set_post_taxonomy()
	...
	public function trail() {
		...
		$breadcrumb = apply_filters( 'travelstore_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			...
			'browse'              => esc_html__( 'Browse:', 'travelstore' ),

			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'travelstore' ),

			'home'                => esc_html__( 'Home', 'travelstore' ),

			'error_404'           => esc_html__( '404 Not Found', 'travelstore' ),

			'archives'            => esc_html__( 'Archives', 'travelstore' ),

			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;', 'travelstore' ),

			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'travelstore' ),

			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'travelstore' ),

			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'travelstore' ),
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			...
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'travelstore' ) ) );
		}
	...
	protected function add_minute_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			...
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'travelstore' ) ) );
		}
	...
	protected function add_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			...
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'travelstore' ) ) );
		}
	...
	protected function add_day_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'travelstore' ) ) );

		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'travelstore' ) ) );

		$day = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'travelstore' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'travelstore' ) ) );
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					...
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'travelstore' ) ) ) );
				} // If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag ) {

					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'travelstore' ) ) ) );
				} // If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag ) {

					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'travelstore' ) ) ) );
